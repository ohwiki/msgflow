...
## Codex / PowerShell Notes

- 在 Windows PowerShell 里优先使用 `python`，不要假设有 `python3`
- 在 PowerShell 里优先使用 `curl.exe`，避免命中 `curl` alias
- 默认技能路径可写成 `"$HOME\\.codex\\skills\\markdown-proxy\\..."`，不依赖 `CODEX_HOME` 是否显式设置
...
```bash
curl.exe -sL "https://r.jina.ai/{original_url}" 2>$null
```
...
```bash
curl.exe -sL "https://defuddle.md/{original_url}" 2>$null
```
...
```
默认保存路径：$HOME\Downloads\{title}.md
...
### X/Twitter 帖子
```bash
curl.exe -sL "https://r.jina.ai/https://x.com/username/status/1234567890" 2>$null
```

### 普通网页
```bash
curl.exe -sL "https://r.jina.ai/https://example.com/article" 2>$null
```
...
- 对于超长内容，可先保存后用 `Get-Content <file> | Select-Object -First 200` 预览前 200 行
